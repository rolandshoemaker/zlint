...
func MakeIssuerString(cert *x509.Certificate, result *zlint.ResultSet, validationInterface interface{}) string {
	...
	outputString += strconv.Itoa(numErrors) + "," + strconv.Itoa(numWarnings) + "," + strconv.FormatBool(validation.nssValid) + "," + strconv.FormatBool(validation.nssWasValid) + "," + notBefore + "," + notAfter + "," + issuerDn + "," + subjectDn + "," + sha256fp + "," + strings.Join(result.Errors, ",") + "," + strings.Join(result.Warnings, ",") + "," + strings.Join(result.Fatals, ",") + "\n"
	return outputString
}
